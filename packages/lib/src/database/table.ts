...
import { v4 } from "uuid";
import { envConfig, nonNullable, nthArg, property } from "..";
import { flatMap } from "../observable";
...
export class Table<R, K extends keyof R> {
  protected static readonly BASE_URL = DATABASE_TABLE_BASE_URL;

  constructor(
    public id: string,
    public key: K,
    public records: TableRecordCreator<R, K>,
    protected observer: NextObserver<TableActionCreator<R, K>>,
  ) {}
    ...
    return new Promise<TableIndex<R, K>>((callback) => {
      switch (args[0]) {
        ...
        case "delete":
          this.observer.next(
            new LazyPromise((tableIndex) =>
              thru(args, ([, id]): TableActionCreatorResult<R, K> => ({
...
interface TableInstance<R, K extends keyof R> {
  new (): Table<R, K>;
}
...
interface FileTableInstance<R, K extends keyof R> {
  new (): Table<R, K> & {
    getFile: (url: string, spinner?: Ora | undefined) => Promise<File>;
  };
}
...
): FileTableInstance<FileTableRecord, "url"> =>
  class extends createTable<FileTableRecord, "url">(id, "url", records) {
