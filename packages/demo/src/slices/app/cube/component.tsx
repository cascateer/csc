...
export const AppCubeComponent = appSlice
  ...
  .withTemplate((ctx, classNames) => () => {
    ...
    animationEnd.pipe(bufferCount(27)).subscribe({
      next: () => ctx.store.actions.cube__incrementCurrentBaseActionIndex(),
    });
            ...
            <div
              ...
              data-cubie-slice-action={combineLatest([
                ctx.terminal.effects
                  .cube__currentBaseAction()
                  .pipe(intersectWith(cubie)),
                ctx.terminal.effects.cube__currentBaseActionParity(),
              ]).pipe(
                map(([action, parity]) => {
                  if (action != null) {
